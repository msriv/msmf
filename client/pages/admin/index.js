  ...
  return (
    <div className="flex m-2">
      <div className="m-4 p-4 w-2/12 flex flex-col space-y-4 bg-deep-cerulean-100 bg-opacity-50 rounded-2xl">
        <div
          onClick={handleMenu}
          className="cursor-pointer self-end w-2/12 transition duration-500 ease-in-out  hover:bg-chambray-200 hover:text-black hover:shadow-lg rounded-xl flex flex-row justify-center items-center p-2"
        >
          <ChevronRight className="w-5" />
        </div>
        <div
          onClick={() => handleComponent(<Dashboard />)}
          className="cursor-pointer transition duration-500 ease-in-out hover:bg-chambray-200 hover:text-black hover:shadow-lg rounded-xl flex flex-row items-center space-x-4 p-2"
        >
